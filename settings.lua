...
function emufun.initConfig()
  ...
  if emufun.config.default_config then return end
  ...
  local config_dir = emufun.config.config_dir
  ...
  for _,file in ipairs(love.filesystem.getDirectoryItems("default")) do
    ...
    if emufun.config.overwrite_config or not lfs.exists(path) then
...
function emufun.loadConfig(name)
  ...
  local path = "%s/%s.cfg" % { emufun.config.config_dir, name }
  if not emufun.config.default_config and lfs.exists(path) then
